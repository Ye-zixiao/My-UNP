...
/**
 * TCP多进程并发服务器（预先分配进程池）：
 * 使用进程共享的互斥量（使用开启进程共享属性的互斥量属性
 * 对象初始化存放在共享内存中的互斥量）来保障accept的原子性
 */
...
/* 使用线程上锁，使得多个进程在使用accept之前都需要对共享的
	互斥量进行加锁 */
void mylock_init(char* pathname) {
	...
	if ((err = pthread_mutexattr_setpshared(&attr, PTHREAD_PROCESS_SHARED)) != 0)
		err_exit(err, "pthread_mutexattr_setpshared error");
	if ((err = pthread_mutex_init(mptr, &attr)) != 0)
		err_exit(err, "pthread_mutex_init error");
	if ((err = pthread_mutexattr_destroy(&attr)) != 0)
		err_exit(err, "pthread_mutexattr_destroy error");
}
